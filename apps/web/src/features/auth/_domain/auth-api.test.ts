...
describe('auth-api', () => {
  ...
  it('posts login payload and returns access token', async () => {
    ...
    const fetchSpy = vi
      .fn()
      .mockResolvedValue(createFetchResponse({ accessToken: 'token-2' }));
    globalThis.fetch = fetchSpy as typeof fetch;
  ...
  it('maps api failure into AuthApiError', async () => {
    const fetchSpy = vi
      .fn()
      .mockResolvedValue(
        createFetchResponse({ error: 'Unauthorized' }, { status: 401 }),
      );
    globalThis.fetch = fetchSpy as typeof fetch;
  ...
  it('calls logout endpoint with cookie credentials', async () => {
    const fetchSpy = vi
      .fn()
      .mockResolvedValue(createFetchResponse({}, { status: 204 }));
    globalThis.fetch = fetchSpy as typeof fetch;
