...
> 相關邏輯詳見 **[12-pro-features.md](./12-pro-features.md)**
...
1. 使用者完成訓練後點擊「打卡」
2. 可選擇綁定當次 session 並填寫簡短心得
3. 系統更新今日打卡與連續天數
4. 返回 `/` 首頁儀表板，看到本週訓練、streak、近期互動摘要

## Scenario J: 好友動態、愛心與簡單個人頁

1. A 完成訓練或打卡後，系統產生 feed event
2. B 進入 `/feed` 看見 A 的動態（若隱私允許）
3. B 可按愛心，A 在動態中看到愛心數增加
4. B 點擊 A 頭像進入 `/users/$userId`，查看簡單個人頁與基本儀表板

## Scenario K: 基礎提醒與分享卡

1. 使用者進入 `/settings/reminders` 設定每日提醒時間
2. 到時間後收到提醒並回到 app 打卡
3. 使用者可在打卡完成頁點「分享卡片」
4. 免費版可選基礎模板；Pro 可選進階模板

---

# Case Study: Example Workout
