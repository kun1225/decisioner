...
1. PR-1 feat(web): auth client and session provider foundation
...
1. PR-2 feat(web): login/register pages and basic auth UX

- 新增 /auth/login、/auth/register
- 表單驗證與錯誤提示
- 登入/註冊成功後導向（含 redirect target）

1. PR-3 feat(web): session restore, guarded routes, and header auth actions

- App 啟動 session restore（refresh -> me）
- 受保護路由 guard
- 全域 header 改為依登入狀態顯示登入/登出與使用者操作

1. PR-4 test: auth integration and critical e2e flows
